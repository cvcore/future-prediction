...
class SepInception(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        super().__init__()
        assert in_channels >= 16, "in_channels to small for SepInception!"

        n_channels = 0
        self.branch0 = nn.Sequential(
            BasicConv3d(in_channels, in_channels//2, 1, 1)
        )
        n_channels += in_channels//2

        self.branch1 = nn.Sequential(
            BasicConv3d(in_channels, in_channels//2, 1, 1),
            SepConv3d(in_channels//2, int(in_channels*1.3), 3, 1, 1)
        )
        n_channels += int(in_channels*1.3)

        self.branch2 = nn.Sequential(
            BasicConv3d(in_channels, in_channels//16, 1, 1),
            SepConv3d(in_channels//16, in_channels//8, 3, 1, 1)
        )
        n_channels += in_channels//8

        self.branch3 = nn.Sequential(
            nn.MaxPool3d((3,3,3), 1, 1),
            BasicConv3d(in_channels, in_channels//6, 1, 1)
        )
        n_channels += in_channels//6

        self.project = BasicConv3d(n_channels, out_channels, 1, 1)
...
class Dynamics(nn.Module):
    ...
    def __init__(self, in_channels, n_frames, layer_channels):

        super().__init__()
        n_layers = len(layer_channels)
        assert n_layers == n_frames - 1, f"Got invalid number of frames = {n_frames} !"

        conv_layers = []
        last_channels = in_channels
        for layer_ch in layer_channels:
            conv_layers.append(
                SepInception(last_channels, layer_ch)
            )
            last_channels = layer_ch
...
if __name__ == "__main__":
    ...
    sepinc = SepInception(80, 88)
    a = torch.zeros(2, 80, 5, 33, 65)
    b = sepinc(a)
    print(b.shape)

    dyn_sepinc = Dynamics(2048, 5, [80, 88, 96, 104]).cuda(1)
    a = torch.zeros(2, 2048, 5, 33, 65).cuda(1)
    b = dyn_sepinc(a)
    print(b.shape)
