...
class BPnP(torch.autograd.Function):
    ...
    def forward(ctx, pts2d, pts3d, K, ini_pose=None):
        ...
        for i in range(bs):
            ...
            if ini_pose is None:
                _, rvec0, T0, _ = cv.solvePnPRansac(objectPoints=pts3d_np, imagePoints=pts2d_i_np, cameraMatrix=K_np, distCoeffs=None, flags=cv.SOLVEPNP_EPNP, confidence=0.9999 ,reprojectionError=1)
            else:
                rvec0 = np.array(ini_pose[i, 0:3].cpu().view(3, 1))
                T0 = np.array(ini_pose[i, 3:6].cpu().view(3, 1))
            _, rvec, T = cv.solvePnP(objectPoints=pts3d_np, imagePoints=pts2d_i_np, cameraMatrix=K_np, distCoeffs=None, flags=cv.SOLVEPNP_EPNP, useExtrinsicGuess=True, rvec=rvec0, tvec=T0)
...
class BPnP_m3d(torch.autograd.Function):
    ...
    def forward(ctx, pts2d, pts3d, K, ini_pose=None):
        ...
        for i in range(bs):
            ...
            if ini_pose is None:
                _, rvec0, T0, _ = cv.solvePnPRansac(objectPoints=pts3d_i_np, imagePoints=pts2d_i_np, cameraMatrix=K_np, distCoeffs=None, flags=cv.SOLVEPNP_EPNP, confidence=0.9, reprojectionError=5, )
            else:
                rvec0 = np.array(ini_pose[i, 0:3].cpu().view(3, 1))
                T0 = np.array(ini_pose[i, 3:6].cpu().view(3, 1))
            _, rvec, T = cv.solvePnP(objectPoints=pts3d_i_np, imagePoints=pts2d_i_np, cameraMatrix=K_np, distCoeffs=None, flags=cv.SOLVEPNP_EPNP, useExtrinsicGuess=True, rvec=rvec0, tvec=T0)
            angle_axis = torch.tensor(rvec,device=device,dtype=torch.float).view(1, 3)
            T = torch.tensor(T,device=device,dtype=torch.float).view(1, 3)
            P_6d[i,:] = torch.cat((angle_axis,T),dim=-1)
...
def get_coefs(P_6d, pts3d, K):
    ...
    torch.set_grad_enabled(True)
    y = P_6d.repeat(n,1)
    proj = batch_project_orig(y, pts3d, K).squeeze()
...
def batch_project(P, pts3d, K, angle_axis=True):
    ...
    bs = pts3d.size(0)
    device = P.device
    pts3d_h = torch.cat((pts3d, torch.ones(bs, n, 1, device=device)), dim=-1)
    if angle_axis:
...
def batch_project_orig(P, pts3d, K, angle_axis=True):
    n = pts3d.size(0)
    bs = P.size(0)
    device = P.device
