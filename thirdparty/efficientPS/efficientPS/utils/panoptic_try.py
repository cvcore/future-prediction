    ...
    def __call__(self, sem_pred, bbx_pred, cls_pred, obj_pred, msk_pred, sem_logits, num_stuff):
        ...
        try:
           ...
           for idx_sub in range(0,idx_len,max_limit):
            ...
            sem_prob = sem_logits.softmax(dim=0)
            # Process instances
            ...
            for id, msk_f, cls_i, obj_i in zip(idx[idx_sub:idx_sub+max_limit], msk_pred1, cls_pred[idx[idx_sub:idx_sub+max_limit]], obj_pred[idx[idx_sub:idx_sub+max_limit]]):
                ...
                if len(sem_pred[msk_i]) != 0:
                 v,_ = torch.mode(sem_pred[msk_i])
#                if v!=cls_i.item()+num_stuff and v>=num_stuff:
#                    print(v,cls_i.item()+num_stuff, obj_i.item(), sem_prob[v,msk_i].mean())
                #mask_energy[i, ...]= msk_f
                 if v==cls_i.item()+num_stuff: 
                    cls_pred_.append(cls_i.item())
                 elif v<num_stuff:
                    continue
                    #cls_pred_.append(v-num_stuff)
                 elif sem_prob[v,msk_i].mean().item()> obj_i.item():
#                    continue 
                    cls_pred_.append(v-num_stuff) 
                 else:    
                    cls_pred_.append(cls_i.item()) 
           ...
           mask_energy = mask_energy[:seg_inst_logits.shape[0]] 
